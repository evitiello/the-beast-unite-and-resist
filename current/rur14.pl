...
if ($dev == 1) {
	$update_host			= "perceive";
	$update_referer	= "http://perceive/cloudmakers";
	$update_post_to	= "http://perceive/cloudmakers/client_3.asp";
}
											...
											for ($hex33=$config{entry_33}{start}; $hex33 <= $config{entry_33}{end}; $hex33++) {
												for ($pick3_int=$config{pick_3}{start}; $pick3_int <= $config{pick_3}{end}; $pick3_int++) {
													...
													if ($result_size != 0) {
														...
														# we got a 0. try again.
														if ($tries <= $max_tries) {
															print "\nReceived 0. Trying again ($tries/$max_tries).\n";
															$hex23--;
														} else {
															print "giving up on this one.  next!";
														}
													}
												}
											}
